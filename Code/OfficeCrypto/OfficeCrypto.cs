...
namespace OfficeOpenXmlCrypto
{
	...
	public class OfficeCrypto
	{
		...
		public void EncryptPackage(byte[] packageContents, string password, out byte[] encryptionInfo, out byte[] encryptedPackage)
		{
			...
			// Generate a salt
			System.Security.Cryptography.Aes aes = Aes.Create();
			saltSize = 0x10;
		...
		internal byte[] DecryptInternal(string password, byte[] encryptionInfo, byte[] encryptedPackage)
		{
			...
			{
				BinaryReader reader = new BinaryReader(ms, Encoding.UTF32);
				...
				// Encryption header
				uint headerLength		= reader.ReadUInt32();//.ReadUInt32(); 
		...
		{
			System.Security.Cryptography.SHA1 sha1 = SHA1.Create();
		...
		private byte[] AESEncrypt(byte[] data, byte[] key)
		{
			...
			//  Create uninitialized Rijndael encryption object.
			System.Security.Cryptography.Aes symmetricKey = Aes.Create();
